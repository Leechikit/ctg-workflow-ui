    ...
    watch: {
      'soul.model.required.value'(n){
       ...
       const validatePassCheck = (rule, value, callback) => {
                ...
                } else if (this.soul.model['校验类型'].value == 'phone') {
                   var re=/^1\d{10}$/;  
                   if(re.test(value)){  
                       callback();
                   }else{  
                       callback(new Error('手机号填写格式不正确'));
                   }  
                } else if(this.soul.model['校验类型'].value == 'post'){
                    var re= /^[1-9][0-9]{5}$/
                    if(re.test(value))
                          callback();
                    else{
                      callback(new Error('邮政编码填写格式不正确'));
                    }
                } else if(this.soul.model['校验类型'].value == 'post'){
                     re= /\w@\w*\.\w/
                     if(re.test(value))
                      callback();
                       ...
                       callback(new Error('邮箱格式填写不正确'));
                     }
                } else if(this.soul.model['校验类型'].value == 'identification'){
                  
                  if(this.IdentityCodeValid(value))
                      callback();
                     else{
                       callback(new Error('身份证格式填写不正确'));
                  }
                }
                else{
                    callback();
