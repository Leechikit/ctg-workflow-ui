...
$.extend(true, myflow.editors, {
	...
	buttonEditor: function (buttonId,text,fun) {
		var _props,_k,_div,_src,_r;
		this.init = function (props, k, div, src, r) {
			_props=props; _k=k; _div=div; _src=src; _r=r;
			$('<button id = "' + buttonId + '">' + text + '</button>').appendTo('#'+_div);
			$('#'+_div).data('editor', this);
			$('#'+ buttonId).bind('click',[props, k],fun);
		};
		this.destroy = function(){
			$('#'+_div+' input').each(function(){
				_props[_k].value = $(this).val();
	...
	textAreaEditor: function(argument) {
		var _props,_k,_div,_src,_r;
		this.init = function(props, k, div, src, r){
			_props=props; _k=k; _div=div; _src=src; _r=r;
			
			$('<textarea style="width:100%;height:50px"/>').val(props[_k].value).change(function(){
				props[_k].value = $(this).val();
			}).appendTo('#'+_div);
