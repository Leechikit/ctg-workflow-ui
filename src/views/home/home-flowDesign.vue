...
<template>
    <div id="appDesign">
        ...
        <div id="contentCurrent" class="clearfix" ref="content">
            <!-- 之前保存的应用模块 -->
            <div v-for="(item, index) in appData" :key="item.Code" class="content_box_current content_normal" :data-item="item.Code" :class="'content_box_color'+(index+1)%6" :style="{zIndex: (900-index)}"
            @mouseover="showBottomBox(index)" @mouseout.stop.prevent="hideBottomBox(index)" @click.stop="toApp(item.id, item.moduleName)">
                ...
                <Poptip width="101" placement="bottom-start" class="configBtnPanel" :style="{zIndex: 100000}">
                    <a v-if="item.Code !== 'Sys_Workflow'" class="setting-icon-box" @click="showHideIcon(index)">
                        <Icon type="gear-a"></Icon>
                    </a>
                    <div class="operateBtns" slot="content">
                        <a class="boxEdit"  @click.stop.prevent="modModule(item.id,item.moduleName)">修改</a>
                        <a class="boxDel"  @click.stop.prevent="delApp(item.id)" style="color:red;">删除</a>
                    </div>
                </Poptip>
            </div>
            <!-- 新建应用 -->
        ...
        newModule() {
            this.$Modal.confirm({
                ...
                onOk: () => {
                    ...
                    HTTP.createModule(this.addParam,this.addIcon)
                    .then(res => {
