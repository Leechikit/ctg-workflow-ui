...
jQuery.extend({
  ...
  ControlMana: function (sheetDesigner, dataField, settings) {
    ...
    // 加载控件属性
    this.LoadPropertySetting = function () {
      for (var i = 0; i < this.DesignProperties.length; ++i) {
        ...
        switch (designProperty.Name) {
          ...
          pManager = this.RenderTextBox(designProperty, function (val) {
            ...
            } else if (that.ControlKey === "FormLabel" && that.DataField != "CreatedTime") {
          ...
          case "BOSchemaCode":
            ...
            if ((this.ControlKey == "FormDropDownList" && this.Settings["DataSource"] == "Association") || (this.ControlKey == "FormQuery") || (this.ControlKey == "FormMultiQuery")) {
              pManager.Show();
            }
            else {
              pManager.Hide();
          ...
          pManager = this.RenderBOSchemaField(designProperty);
          break;
            // case "CameraOnly":
    ...
    this.RenderUnitSelectionRange = function (designProperty) {
      ...
      if (this.Settings["UnitSelectionRange"]) {
        ...
        $.ajax({
          cache: false,
          url: baseUrl + "/Form/OnAction",
          type: "POST",
          data: { PostData: JSON.stringify({ ActionName: 'SheetUserAction', Command: 'LoadUnits', UnitIDs: unitSelectionRange }) },
          dataType: "json",
          async: true,
          success: function (data) {
            if (data.Successful) {
              ...
              for (var i = 0; i < data.length; i++) {
                var $label = $("<label class='label label-info'>" + data[i].DisplayName + "</label>");
                $item.append($label);
              }
    ...
    this.RenderPrefix = function (designProperty, callback) {
      ...
      var defaultValue = this.Settings[designProperty.Name] || designProperty.DefaultValue;
      var $item = $("<input maxlength=4 class='property-input'>").val(defaultValue);
      //初始渲染时候也要更新预览
      if (callback && $.isFunction(callback)) {
    ...
    this.RenderDropDownDataSource = function (designProperty, callBack) {
      ...
      var $div = $("<div></div>");
      $div.append($itemSelfDe).append($itemFromForm);
      propertyManager.AddItem($div);
    ...
    this.ShowDataRuleModal = function () {
      ...
      if (that.Settings["BOSchemaCode"] == void 0 || that.Settings["BOSchemaCode"] == "") {
        ...
        that.Designer.LoadPublishedSchemas(schemaCodes, function () {
          ...
          if (that.BoSchema != void 0) {
            that.Designer.LoadPublishedSchemaProperty(that.BOSchemaCode);
            if (!$.isEmptyObject(that.Designer.SchemaProperties)) {
          ...
          var $sheetControls = that.Designer.$SheetContent.find(".sheet-control[data-controlkey!='FormAssociateProperty'][data-controlkey!='FormLabel'][data-controlkey!='FormGridView'][data-controlkey!='FormSns'][data-controlkey!='FormBoList'][data-controlkey!='FormMap'][data-controlkey!='FormFormula']");
    ...
    //更新关联属性配置选择字段
    this.UpdateMappingFields = function (schemaCode, $item) {
      var that = this;
      ...
      that.Designer.LoadPublishedSchemaProperty(schemaCode);
      if (!$.isEmptyObject(that.Designer.SchemaProperties)) {
        var properties = that.Designer.SchemaProperties[schemaCode];
        if (properties != void 0 && properties.length > 0) {
          for (var i = 0; i < properties.length; i++) {
            if (properties[i].DataType == $.BizDataType.Attachment) {
              //过滤掉图片/附件
              continue;
            }
    ...
    //关联表单字段/用于关联配置
    this.RenderBOSchemaField = function (designProperty, callBack) {
      var that = this;
      var propertyName = designProperty.Name;
      var propertyManager = new $.PropertyManager(designProperty.Name);
      propertyManager.SetTitle(designProperty.Text);
      ...
      var defaultValue = this.Settings[designProperty.Name] || designProperty.DefaultValue;
      var $div = $("<div></div>");
      var $schemaSelector = $('<select class="form-control" style="display: none;"><option value="" data-boschemacode="">���择关联表单</option></select>');
      ...
      if (boField && boField != "") {
        ...
        //加载字段到item2
        that.UpdateMappingFields(existMappingSchemaCode, $propertySelector);
