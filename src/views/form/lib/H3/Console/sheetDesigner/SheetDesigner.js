...
window.SheetDesigner = function (sheetCode, ajaxUrl, engineCode, dataLoadedCallback) {
...
(function (SheetDesigner) {
  ...
  SheetDesigner.prototype = {
    ...
    initChangeBtn: function () {
      ...
      if ($('.layoutrow').find('.btn-change').length == 0) {
        $('.layoutrow').append($('<span data-type="changeplace" title="点击交换位置" class="btn-change icon-exchange" style="left:' + widthLeft + 'px"></span>'));
      }
    },
    ...
    Release: function (validateCode) {
	  ...
	  if($.BizSheet.SubmitRules == null){
		$.BizSheet.SubmitRules = new Array();
	  }
      ...
      // this.Requst(this.Action_Release, {
      //   SheetCode: that.SheetCode,
      //   BizSheetStr: JSON.stringify($.BizSheet),
      //   SchemaStr: JSON.stringify($.Schema),
      //   BoSchemaCodeStr: JSON.stringify($.BoSchemaCodes)
    ...
    BuildData: function (unValidate) {
      ...
      console.log($.BizSheet.DesignModeContent);
      $.Schema.Properties = {};
        ...
        if ($row.hasClass(this.PageHeader) || $row.hasClass(this.DesControl)) {
          ...
          if ($row.hasClass("SheetGridView")) {
            ...
            if (ctrlKey == "FormSeqNo") {
              ...
              $.Schema.SeqNoLength = $.Schema.SeqNoPrefix.length + ($.Schema.SeqNoDateFormat == "None" ? 0 : $.Schema.SeqNoDateFormat.length) + parseInt(Settings["IncrementNum"]);
            }
            if (ctrlKey == "FormQuery") {
              hasFormQuery = true;
            }
    ...
    ShowNameSummarySelector: function ($ItemEl, type) {
      ...
      var rows = that.$SheetContent.children("div.row:not(.SheetGridView):not(." + this.PageHeader + "):not(." + this.DesControl + "):not([data-controlkey='FormBoList']):not([data-controlkey='FormSns']):not([data-controlkey='FormAttachment']):not([data-controlkey='FormPhoto']):not([data-controlkey='FormButton']):not([data-controlkey='FormFormula']):not([data-controlkey='FormAssociateProperty'])");
      ...
      for (var i = 0; i < rows.length; i++) {
        ...
        if ($el.hasClass("sheet-control")) {
          ...
          //布局控件
          var $leftControl = $el.find("div[data-layoutitem='Left']").children("div.sheet-control:not([data-controlkey='FormBoList']):not([data-controlkey='FormSns']):not([data-controlkey='FormAttachment']):not([data-controlkey='FormPhoto']):not([data-controlkey='FormButton']):not([data-controlkey='FormFormula']):not([data-controlkey='FormAssociateProperty'])");
          var $rightControl = $el.find("div[data-layoutitem='Right']").children("div.sheet-control:not([data-controlkey='FormBoList']):not([data-controlkey='FormSns']):not([data-controlkey='FormAttachment']):not([data-controlkey='FormPhoto']):not([data-controlkey='FormButton']):not([data-controlkey='FormFormula']):not([data-controlkey='FormAssociateProperty'])");
          if ($leftControl.length > 0) {
            id = $.IGuid();
            var dataField = this.GetDataField($leftControl);
            //CreateBy.FullName-》CreateBy
    ...
    ShowGridViewNameSelector: function ($GridView, $NameItems) {
      ...
      //主表字段
      var rows = this.$SheetContent.children("div.row:not(.SheetGridView):not(." + this.PageHeader + "):not(." + this.DesControl + "):not([data-controlkey='FormBoList']):not([data-controlkey='FormSns']):not([data-controlkey='FormAttachment']):not([data-controlkey='FormPhoto']):not([data-controlkey='FormButton']):not([data-controlkey='FormAssociateProperty'])");
      for (var i = 0; i < rows.length; i++) {
        var $el = $(rows[i]);
        var id = $.IGuid();
        if ($el.hasClass("sheet-control")) {
          ...
          //布局控件
          var $control = $el.find("div.sheet-control:not([data-controlkey='FormBoList']):not([data-controlkey='FormSns']):not([data-controlkey='FormAttachment']):not([data-controlkey='FormPhoto']):not([data-controlkey='FormButton']):not([data-controlkey='FormAssociateProperty'])");
          if ($control.length > 0) {
      ...
      var ctrls = $GridView.find(".sheet-control:not([data-controlkey='FormBoList']):not([data-controlkey='FormSns']):not([data-controlkey='FormAttachment']):not([data-controlkey='FormButton']):not([data-controlkey='FormAssociateProperty'])");
    ...
    BuildPropertie: function (dataField, Settings) {
      ...
      switch (Settings.ControlKey.toLowerCase()) {
        case "formassociateproperty":
          controlKey = Settings.SourceType || 14;
          break;
        case "formmap":
      ...
      if (dataField.indexOf(".") > -1) {
        ...
        if (Settings.ControlKey.toLowerCase() == "formgridview") {
          ...
          if (Settings.ControlKey.toLowerCase() == 'formquery' ||
            Settings.ControlKey.toLowerCase() == 'formmultiquery') {
    ...
    SortableStop: function (item, event) {
      ...
      if (that.CurrentDrag) {
        ...
        if ($parent.hasClass("sheetContent")) {
          spanwith = 2;
        }
        //添加控件
    ...
    RenderControl: function (item, spanwith) {
      ...
      switch (controlkey) {
        ...
        case this.FormFormula:
          $input.attr("placeholder", "用于展示公式计算结果,且数据不会保存");
          break;
        case "FormAreaSelect":
          //地区控件
        ...
        case this.TowColsKey:
          ...
          $el = null;
          if (!$parent.hasClass("layoutrow-item")) {
            $el = $('<div class="row layoutrow">').attr("data-" + this.ControlKey, controlkey);
    ...
    //执行删除操作
    DoRemoveControl: function ($el, needValidate) {
      var that = this;
      ...
      if (needValidate == void 0 || needValidate == true) {
        //删除校验
        //配置新的规则后删除字段校验做了调整
        if (!that.RemoveControlValidate($el)) {
          return;
        }
        var Actions = [{
          Key: "btn_OK",
          ...
          Theme: 'btn_delete',
          CallBack: function () {
            that.RemoveControl.apply(that, [$el, false]);
    ...
    //删除控件,先判断能不能删除
    RemoveControl: function ($el, needValidate) {
      var that = this;
      var fields = [];
      if (!$el.hasClass("layoutrow")) {
    ...
    //删除控件校验
    RemoveControlValidate: function ($el) {
