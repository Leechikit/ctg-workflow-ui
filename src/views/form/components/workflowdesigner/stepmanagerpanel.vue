                            ...
                            <div class="value">
                                ...
                                <div v-if="$store.state.participant.StepManagerNode">
                                    <template v-if="$store.state.participant.StepManagerNode.NodeType==0">
                                        ...
                                        <span>{{$store.state.participant.StepManagerNode.Name}}</span>
                                        <span class="icon icon-close2" @click="removeCheck()"></span>
                                    </template>
                                    <template v-if="$store.state.participant.StepManagerNode.NodeType==1">
                                        <span>{{$store.state.participant.StepManagerNode.DisplayName}}</span>
                                        <span class="icon  icon-close2" @click="removeCheck()"></span>
                                    </template>
                                </div>
                            </div>
            ...
            ok(){
                ...
                if (node && node.ObjectId) {
                    tmp={
                        ...
                        Formula: 'RECURSIVEMANAGERSOF(u('+node.ObjectId+'),'+this.level+')',
                        DisplayName: 'RECURSIVEMANAGERSOF('+node.Name+','+this.level+')'
                    }
                }else{
                    tmp={
                        NodeType: NodeType.Function,
                        Formula: 'RECURSIVEMANAGERSOF('+node.Name+','+this.level+')',
                        DisplayName: 'RECURSIVEMANAGERSOF('+node.DisplayName+','+this.level+')'
                    }
                }
                this.$emit("ok",tmp);
